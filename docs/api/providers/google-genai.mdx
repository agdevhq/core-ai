...
Google attaches thought signatures to reasoning parts and to function call parts. Both live under the `google` key of the part's `providerMetadata`. Use `getProviderMetadata` to access them in a type-safe way.
...
Signatures on tool calls are read the same way, from `part.providerMetadata` of a `tool-call` part.
