...
On adaptive models, omit `maxTokens` with `effort: 'max'` and the request uses
the model's full output limit instead of the small default allowance. An
explicit `maxTokens` stays a hard limit.
