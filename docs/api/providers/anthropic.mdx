...
- `output.maxTokens` is the verified Messages API ceiling when the registry
  has one (for example 128,000 on Opus 4.6). The field is absent for unknown
  ids.
...
These token budgets are this library's mapping, not Anthropic-documented
effort values: `minimal` -> 1,024, `low` -> 2,048, `medium` -> 8,192,
`high` -> 32,768, `max` -> 65,536.
...
Reasoning shares the output allowance with the visible answer.

On adaptive models, omit `maxTokens` and the request uses the model's full
output limit instead of the small default allowance. Effort controls how much
of that allowance the model spends. An explicit `maxTokens` stays a hard limit.

On Claude 4.5 and earlier, the thinking budget shrinks to fit `maxTokens`.

The Anthropic SDK rejects non-streaming requests above model-specific
thresholds. The lowest threshold is 8,192 tokens, so `generate()` and
`generateObject()` with a larger `max_tokens` use the streaming transport and
still return a regular result.
