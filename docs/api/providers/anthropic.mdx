...
Anthropic's schema normalization applies to all tools, strict or not. Strict
tool schemas do not change Anthropic's structured response format for
`generateObject()` and `streamObject()`. See
