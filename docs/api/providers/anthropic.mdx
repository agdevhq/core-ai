...
<AccordionGroup>
  ...
  <Accordion title="Claude Opus and Sonnet (adaptive thinking)" icon="brain">
    ...
    <ul>
      <li><strong>claude-opus-5-5</strong> - Current Opus model for long-running agentic coding and knowledge work. Adaptive thinking is always on</li>
      <li><strong>claude-opus-5</strong> - Opus model for complex agentic coding and enterprise work</li>
...
- `mode` is `always-on` for `claude-opus-5-5`, because adaptive thinking
  cannot be disabled. For every other registered Claude model, `mode` is
  `optional` and reasoning is enabled by passing `reasoning`.
...
- `supportedToolChoices` is `['auto', 'none']` while reasoning is enabled. On
  `claude-opus-5-5` that restriction applies to every request.
...
<Note>
    ...
    `claude-opus-4-6`, `claude-sonnet-5`, and `claude-sonnet-4-6` support
    `'max'` effort directly. On `claude-opus-5-5`, thinking cannot be disabled:
    omit `reasoning` to use the API default effort of `medium`, or pass
    `reasoning.effort` to choose a level. `thinking` is sent as `adaptive`.
</Note>
...
Claude Opus 5.5, Claude Fable 5, Mythos 5, Mythos Preview, Opus 5, Opus 4.8,
Opus 4.7, and Sonnet 5 also reject non-default sampling parameters when you do
not explicitly enable reasoning. For these models, use `temperature: 1`,
`topP: 1`, and omit `topK`.

Claude Opus 5.5 also rejects `toolChoice: 'required'` and
`toolChoice: { type: 'tool', toolName }` on every request. `'auto'` and
`'none'` remain valid.
...
- 512 tokens for Claude Opus 5.5
