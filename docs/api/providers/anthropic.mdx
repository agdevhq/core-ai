...
Anthropic spends thinking tokens out of `max_tokens` and requires the budget to
stay below it, so `supportedEfforts` only lists the efforts whose budget fits
under the model's output limit. `claude-opus-4-5`, `claude-sonnet-4-5`, and
`claude-haiku-4-5` cap out at `high`, and `claude-opus-4-1` at `medium`. A
higher requested effort is clamped down to the highest one the model can run.
...
Omit `maxTokens` and the request is sized for the effort you asked for: the
thinking budget plus the provider's `defaultMaxTokens` as the answer
allowance, capped at the model's output limit. Adaptive `max` effort gets the
model's full output limit, since adaptive thinking has no separate budget to
reserve.

Set `maxTokens` and it stays a hard limit. A limit that cannot hold the
requested thinking budget raises a `ValidationError` instead of silently
reducing the budget, so a request for deep reasoning never quietly turns into
a shallow one. Raise `maxTokens`, lower the effort, or omit `maxTokens`
entirely.
