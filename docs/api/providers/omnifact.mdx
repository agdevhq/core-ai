...
normalized schema; other tools are sent unchanged. Whether enforcement happens
depends on the routed backend — backends without strict support reject the
request. See
...
The gateway follows the OpenAI Chat Completions API. Pass options via `providerOptions.omnifact`:
