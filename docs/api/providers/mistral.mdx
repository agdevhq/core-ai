...
Mistral's API accepts a `strict` flag but does not document enforcement, so
every model reports `model.capabilities.tools.strictSchemas` as
`{ supported: false }` and `strict: true` throws `ToolSchemaStrictnessError`
locally. See
