...
- `modalities.input` / `modalities.output` are both `['text']`; both are
  text-only models.
...
Kimi models report `model.capabilities.tools.strictSchemas` as
`{ supported: true }`. Tools with `strict: true` are forwarded per tool with
`strict: true` and a normalized schema; other tools are sent unchanged. The
endpoint rejects strict tools it cannot enforce. See
[Strict tool schemas](/guides/tool-calling#strict-tool-schemas) for the schema
contract and error reasons.

This tool behavior is separate from structured output. `generateObject()` and
`streamObject()` use JSON Mode and then validate the returned object with Zod.
