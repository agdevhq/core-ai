...
Tools with `strict: true` are forwarded per tool with `strict: true` and a
normalized schema; other tools are sent unchanged. Endpoints that do not
implement OpenAI strict function tools reject or ignore the field, so opt
tools in only when the target endpoint supports the OpenAI contract. See
...
Pass Chat Completions options through `providerOptions['openai-compat']`:
