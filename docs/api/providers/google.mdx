...
description: 'Create and configure the Google provider for Gemini models with multimodal capabilities'
---

## Overview

The Google provider gives you access to Gemini models with advanced multimodal capabilities, embeddings, and image generation through Imagen.

## Installation

```bash
npm install @core-ai/google
```
...
<ParamField path="client" type="GoogleClient" optional>
    Provide your own configured Google client instance.
</ParamField>

### Returns

`GoogleProvider` with methods `chatModel()`, `embeddingModel()`, and `imageModel()`.
...
When reasoning is enabled, Google reasoning parts include a thought signature for multi-turn fidelity. Use `getProviderMetadata` to access it in a type-safe way.

```typescript
import { generate, getProviderMetadata } from '@core-ai/core-ai';
import type { GoogleReasoningMetadata } from '@core-ai/google';
