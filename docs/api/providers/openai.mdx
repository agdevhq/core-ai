...
<Note>
    ...
    `chatModel()` (the Responses API). GPT-6 Sol and Luna support Chat
    Completions function calling. The adapter forwards tools and `reasoning`
    as you set them, the same as GPT-5.6. OpenAI accepts function tools on
    these models only when the effective reasoning effort is `none`. Omitting
    `reasoning` leaves the provider default, `medium`.
</Note>
