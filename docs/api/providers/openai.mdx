...
strict tools when the model lacks support. Two restrictions apply:

- The Responses API does not support strict function tools on
  `gpt-4o-2024-05-13`, GPT-4 Turbo, or tool-capable GPT-3.5 Turbo models. The
  Responses model factory reports them as `{ supported: false }`; Chat
  Completions (`openai.chat.chatModel()`) supports them.
- Audio models report `{ supported: false }` through both APIs.
