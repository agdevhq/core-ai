...
<AccordionGroup>
  <Accordion title="GPT-6 Series" icon="star">
    <ul>
      <li><strong>gpt-6-astra</strong> - Flagship model for the hardest reasoning, coding, and computer-use work</li>
      <li><strong>gpt-6-sol</strong> - Strong reasoning model for demanding coding and agentic workflows</li>
      <li><strong>gpt-6-luna</strong> - Efficient model for focused, high-volume tasks</li>
    </ul>
  </Accordion>
...
<Accordion title="o-Series (Reasoning Models)" icon="brain">
    <ul>
        <li>
            <strong>o4-mini</strong> - Compact reasoning model
        </li>
        <li>
            <strong>o3-pro</strong> - o3 model with more compute for harder
            reasoning tasks
        </li>
        <li>
            <strong>o3</strong> - Advanced reasoning capabilities
        </li>
        <li>
            <strong>o3-mini</strong> - Efficient reasoning model
        </li>
        <li>
            <strong>o1</strong> - First-generation reasoning model
        </li>
        <li>
            <strong>o1-mini</strong> - Compact reasoning model with no effort
            control
        </li>
    </ul>
</Accordion>
...
`ChatModel.capabilities` describes the selected model and API. The exported
`getOpenAIModelCapabilities(modelId)` helper returns the base Chat Completions
capabilities. The Responses model factory applies its additional API-specific
restrictions.

### Strict tool schemas

Tool-capable OpenAI models report `model.capabilities.tools.strictSchemas` as
`{ supported: true }`. Fine-tuned `ft:<base>:...` models inherit the base
model's capabilities. Unrecognized model IDs resolve optimistically to
supported and rely on OpenAI to reject strict tools when the model lacks
support. Two restrictions apply through both the Responses and Chat
Completions APIs:

- Models that predate Structured Outputs — `gpt-4o-2024-05-13`, GPT-4 Turbo,
  and GPT-3.5 Turbo — report `{ supported: false }`.
- Audio models report `{ supported: false }`.

Non-strict tools are sent to the Responses API with an explicit `strict: false`
so the API does not normalize them into strict mode on its own.

OpenAI does not guarantee schema adherence when parallel tool calls are
enabled. Set `providerOptions.openai.parallelToolCalls: false` on requests
that combine strict tools with parallel tool calls.

Strict function tools are independent of the JSON Schema response formats used
by `generateObject()` and `streamObject()`. See
[Strict tool schemas](/guides/tool-calling#strict-tool-schemas) for the schema
contract and error reasons.
...
`reasoning.mode` is `always-on` for `gpt-6-astra`, `gpt-6-sol`, and
`gpt-6-luna`. Those models reason unless a Chat Completions request explicitly
disables reasoning. Other OpenAI chat models are `optional` or `unsupported`.
`supportedEfforts` lists the accepted effort levels, and `supportedToolChoices`
contains all unified tool-choice modes. `restrictsSamplingParams` identifies
models that reject `temperature` and `topP` while reasoning is enabled.
...
`modalities.input` is `['text', 'image', 'file']` for the GPT-4, GPT-5, and
GPT-6 families, `o1`, `o3`, `o3-pro`, and `o4-mini` (image sources: `base64` and
...
| ---------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`                                                                         | `low`, `medium`, `high`, `max`     |
...
<Warning>
    When `capabilities.reasoning.restrictsSamplingParams` is `true`, the adapter
    throws if you set `temperature` or `topP` while reasoning is enabled. GPT-6
    models reason by default, so those parameters are rejected even when you
    omit `reasoning`.
</Warning>

<Note>
    For GPT-6, `max` is sent as `max`. For GPT-5 and o-series models, `max` is
    sent as `xhigh`. `minimal` on GPT-6 is clamped to `low`.

    GPT-6 Astra does not support tool calling on Chat Completions. Use
    `chatModel()` (the Responses API). GPT-6 Sol and Luna accept Chat
    Completions function calling only when reasoning is disabled. If you pass
    tools and omit `reasoning`, the Chat Completions adapter sends
    `reasoning_effort: "none"` and then allows `temperature` and `topP`.
    Combining tools with `reasoning` on Chat Completions throws. Use the
    Responses API to keep reasoning alongside tools.
</Note>
