...
`{ supported: true }`. Unrecognized model IDs, including fine-tuned `ft:...`
models, resolve optimistically to supported and rely on OpenAI to reject
strict tools when the model lacks support. Two restrictions apply through
both the Responses and Chat Completions APIs:
