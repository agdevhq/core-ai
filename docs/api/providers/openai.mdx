...
`modalities.imageInput.supported` is `true` for the GPT-4 and GPT-5 families, `o1`, `o3`,
`o3-pro`, and `o4-mini`, each accepting `base64` and `url` sources. It is
`false` for the text-only `gpt-3.5-turbo`, `o1-mini`, and `o3-mini`. Model IDs
outside the registry are treated as image capable.
