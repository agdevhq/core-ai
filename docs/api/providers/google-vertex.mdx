...
Gemini 2.5 budgets come from each model's documented range, an omitted
`maxTokens` uses the model's full output limit so thoughts and answer both fit,
and a `maxTokens` that cannot hold the thinking budget raises a
`ValidationError`. See
