...
Gemini 2.5 budgets come from each model's documented range. On known Gemini 2.5
and Gemini 3 models, omitting `maxTokens` while reasoning is set uses the
verified 65,536 output limit. A `maxTokens` that cannot hold a Gemini 2.5
thinking budget raises a `ValidationError`. See
