...
The Google on Vertex AI provider gives you access to Gemini chat and embedding models and Imagen image generation through Google Cloud. It uses the same model behavior as `@core-ai/google`, while configuring the Google SDK for Vertex AI authentication and regional endpoints.
...
Provider options remain namespaced under `google`, matching `@core-ai/google`:
...
The Google provider-option schemas, `GoogleReasoningMetadata`, and `getGoogleModelCapabilities()` are exported by `@core-ai/google` and apply to Vertex-hosted models.
