...
Error for provider/API failures. Always includes a stable `code`. Subclasses add typed metadata for common cases.
...
<ParamField path="code" type="ProviderErrorCode">
  Stable machine-readable classification. Defaults to `'unknown'`.
</ParamField>
...
<ParamField path="isRetryable" type="boolean">
  `true` for `rate_limit_exceeded`, `model_overloaded`, and `service_unavailable`.
</ParamField>

### Classified subclasses

| Class | Fixed `code` | Extra fields |
|-------|--------------|--------------|
| `ContextLengthExceededError` | `context_length_exceeded` | `maxTokens?`, `actualTokens?` |
| `RateLimitError` | `rate_limit_exceeded` | `retryAfterSeconds?` |
| `ModelOverloadedError` | `model_overloaded` | — |
| `ServiceUnavailableError` | `service_unavailable` | — |

Provider packages call `classifyProviderError(context, signals)` so every wrapper shares the same precedence: abort → context length → overload → rate limit → service unavailable → HTTP status fallbacks (`529`, `429`, `500`/`502`/`503`/`504`) → `unknown`. Wrappers should compute `signals` through helpers rather than inlining evaluation in the call.
...
try {
  ...
  if (error instanceof ContextLengthExceededError) {
    ...
    console.error(`Provider ${error.provider} [${error.code}]:`, error.message);
    if (error.isRetryable) {
      console.error('Transient failure — retry with backoff');
    }
  }
}
```
