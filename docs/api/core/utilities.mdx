...
The `source` parameter accepts an `AsyncIterable<StreamEvent>` or a `() => Promise<AsyncIterable<StreamEvent>>` factory function. Pass `{ signal }` to abort the stream. Pass `{ mapError }` so in-band source failures become the same typed errors as a failed initial request. Already-typed core-ai errors pass through unchanged.
