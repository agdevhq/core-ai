...
| `output.maxTokens`                   | Verified ceiling for a single response, in tokens                    |
...
`output` is present only when the provider registry has a verified response
ceiling for that model id. Adapters that spend reasoning out of the same
allowance (Anthropic `max_tokens`, Google `maxOutputTokens`) use it to size
an omitted limit. Callers can read `capabilities.output?.maxTokens` instead of
hardcoding a per-model cap. Treat a missing field as unknown — do not invent a
ceiling, and keep an explicit `maxTokens` if you need a guaranteed floor.
