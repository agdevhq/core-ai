...
| `modalities.imageInput`             | Whether the model accepts image parts in user messages               |
...
Sending an image when it is `false` throws a `ValidationError` before the
provider call. Unrecognized model IDs are treated as image capable on purpose
(fail-open), so self-hosted endpoints and newly released models are never
blocked locally. Typos and custom deployment names that do not match a registry
key therefore report `modalities.imageInput: true` and still rely on the
provider to reject unsupported requests. For Azure and other wrappers, pass a
known registry model ID when you need accurate capability data.
