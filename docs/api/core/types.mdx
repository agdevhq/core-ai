...
type ModelCapabilities = {
    ...
    modalities: {
        imageInput: {
            ...
            supportedSources: readonly ('base64' | 'url')[];
        };
    };
};
```
...
| `modalities.imageInput.supported`   | Whether the model accepts image parts in user messages               |
| `modalities.imageInput.supportedSources` | Image source types the model accepts (empty when unsupported)   |
...
`modalities.imageInput` describes whether the model accepts [image parts](/guides/multi-modal).
Sending an image to a model where `supported` is false, or using a source type
outside `supportedSources`, throws a `ValidationError` before the provider call.
Unrecognized model IDs are treated as image capable on purpose (fail-open), so
self-hosted endpoints and newly released models are never blocked locally.
Typos and custom deployment names that do not match a registry key therefore
report `modalities.imageInput.supported: true` and still rely on the provider to reject
unsupported requests. For Azure and other wrappers, pass a known registry model
ID when you need accurate capability data.
