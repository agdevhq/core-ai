...
| `imageInput.supported`              | Whether the model accepts image parts in user messages               |
| `imageInput.supportedSources`       | Image source types the model accepts (empty when unsupported)        |
...
`imageInput` describes whether the model accepts [image parts](/guides/multi-modal).
...
report `imageInput.supported: true` and still rely on the provider to reject
