...
Not every model accepts images. Read `model.capabilities.modalities.input` to find out before you build a request:
...
Sending image or file parts to a model that does not support them throws an
...
Model IDs a provider does not recognize are treated as multimodal capable, so self-hosted and newly released models are never blocked. Vision-capable models accept both `base64` and `url` image sources.
