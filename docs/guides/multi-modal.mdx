...
Not every model accepts images. Read `model.capabilities.imageInput` to find out before you build a request:

```typescript
const model = openai.chatModel('gpt-4o-mini');

console.log(model.capabilities.imageInput);
...
<AccordionGroup>
  ...
  <Accordion title="Check image support before building a request">
    ...
    ```typescript
    const { supported, supportedSources } = model.capabilities.imageInput;
