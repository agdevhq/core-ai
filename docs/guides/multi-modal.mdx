...
Model IDs a provider does not recognize are treated as image capable, so self-hosted and newly released models are never blocked. Vision-capable models accept both `base64` and `url` image sources.
...
<AccordionGroup>
  ...
  <Accordion title="Check image support before building a request">
    ...
    ```typescript
    const content = model.capabilities.modalities.imageInput
      ? [
          { type: 'text', text: 'Describe this image' },
