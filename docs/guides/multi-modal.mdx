...
Not every model accepts images. Read `model.capabilities.modalities.imageInput` to find out before you build a request:

```typescript
const model = openai.chatModel('gpt-4o-mini');

console.log(model.capabilities.modalities.imageInput);
// { supported: true, supportedSources: ['base64', 'url'] }
```

| Field              | Meaning                                                |
| ------------------ | ------------------------------------------------------ |
| `supported`        | Whether the model accepts image parts in user messages |
| `supportedSources` | Which sources it accepts: `'base64'`, `'url'`, or both |

Sending an image to a model that does not support it throws a `ValidationError` before the request reaches the provider, as does using a source type outside `supportedSources`:
...
try {
  ...
  if (error instanceof ValidationError) {
    // openai model "gpt-3.5-turbo" does not support image input
  }
}
```

Model IDs a provider does not recognize are treated as image capable, so self-hosted and newly released models are never blocked.

To run the same check yourself before dispatching a request, use `validateImageInput`:

```typescript
import { validateImageInput } from '@core-ai/core-ai';
...
<AccordionGroup>
  ...
  <Accordion title="Check image support before building a request">
    ...
    ```typescript
    const { supported, supportedSources } = model.capabilities.modalities.imageInput;

    const content = supported && supportedSources.includes('url')
      ? [
          { type: 'text', text: 'Describe this image' },
