---
title: Multi-Modal
description: Work with images, files, audio, and multi-part messages in chat conversations
---

core-ai supports multi-modal inputs, allowing you to include images, files,
audio, and text in the same message.
...
Google Gemini chat models accept audio through `inlineData`. OpenAI accepts
...
<Note>
    File support varies by provider and model. Check your provider's
...
Not every model accepts every input type. Read
`model.capabilities.modalities.input` before you build a request:

```typescript
import { supportsInputModality } from '@core-ai/core-ai';

const model = openai.chatModel('gpt-4o-mini');

console.log(model.capabilities.modalities.input);
// ['text', 'image', 'file']

console.log(supportsInputModality(model.capabilities, 'image'));
// true
```

Sending image, file, or audio parts to a model that does not support them throws an
`UnsupportedInputModalityError` (a `ValidationError`) before the request
reaches the provider:

```typescript
import { UnsupportedInputModalityError } from '@core-ai/core-ai';
...
try {
    ...
    if (error instanceof UnsupportedInputModalityError) {
        // openai model "gpt-3.5-turbo" does not support input modality: image. Supported: text
        console.log(error.unsupportedModalities);
    }
}
```

Provider registries generally allow image and file input for unrecognized model
IDs so self-hosted and newly released models are not blocked locally. Audio
remains opt-in. Vision-capable models accept both `base64` and `url` image
sources.

To run the same check yourself before dispatching a request, use `validateInputModalities`:

```typescript
import { validateInputModalities } from '@core-ai/core-ai';
...
<AccordionGroup>
  ...
  <Accordion title="Check image support before building a request">
    ...
    const content = supportsInputModality(model.capabilities, 'image')
      ? [
          { type: 'text', text: 'Describe this image' },
          { type: 'image', source: { type: 'url', url: imageUrl } },
        ]
      : [{ type: 'text', text: 'Describe the product at ' + imageUrl }];
    ```
