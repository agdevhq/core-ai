...
<Note>
  `temperature`, `maxTokens`, and `topP` are top-level options. `stopSequences`, `frequencyPenalty`, and `presencePenalty` are provider-specific and passed via `providerOptions`. OpenAI supports them through `openai.chat.chatModel()` and `@core-ai/openai-compat`, not through Responses models. See [Configuration](/concepts/configuration) for details.
</Note>
