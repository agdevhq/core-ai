...
Every `ProviderError` has a stable machine-readable `code`. Prefer switching on `code` for logging and serialization; use subclasses for typed metadata and same-process `instanceof` checks.
...
| Code | Subclass | Meaning | `isRetryable` |
|------|----------|---------|---------------|
| `context_length_exceeded` | `ContextLengthExceededError` | Input/context exceeds model limits | no |
| `rate_limit_exceeded` | `RateLimitError` | HTTP 429 / provider rate limit | yes |
| `model_overloaded` | `ModelOverloadedError` | Capacity / high-demand signal | yes |
| `service_unavailable` | `ServiceUnavailableError` | Temporary unavailability (e.g. 5xx) | yes |
| `unknown` | `ProviderError` | Unclassified provider failure | no |

**Classification precedence** (shared by every provider via `classifyProviderError`):

1. Abort / cancel → `AbortedError` (not a `ProviderError`)
2. Context length
3. Model overloaded (explicit capacity signal, e.g. Anthropic HTTP 529 / `overloaded_error`, or high-demand copy on 5xx/529)
4. Rate limit (HTTP 429, provider rate-limit types)
5. Service unavailable (explicit unavailable signal)
6. HTTP status fallbacks in the same order: `529` → overload, `429` → rate limit, `500`/`502`/`503`/`504` → unavailable
7. Everything else → `code: 'unknown'`

Provider `wrap*Error` helpers build a small context (`message`, `provider`, `statusCode`, `cause`) and a separate signals object via local helpers — they should not inline compound classification logic in the `classifyProviderError` call. Classification itself is centralized so all providers share the same importance order. Explicit signals beat status fallbacks, which is how quirks like Azure capacity returning HTTP 429 can map to `service_unavailable` instead of a rate limit. Overload message cues are only applied on capacity-like statuses (`500`/`502`/`503`/`504`/`529`), never on ordinary 4xx including `429`.

Every provider wrapper covers the same signal categories at the same depth: abort, context length (with or without token counts), overload, rate limit (including `Retry-After` when present), service unavailable / transient 5xx, and unknown. Provider-specific details (error codes, message shapes, Azure/Vertex quirks) differ, but no provider should have a deeper classification surface than the others.
...
try {
  ...
  if (error instanceof ProviderError) {
    ...
    if (error.isRetryable) {
      console.log('Transient failure — safe to retry with backoff');
    }
  }
}
```
...
- `RateLimitError` — optional `retryAfterSeconds` from `Retry-After`
- `ModelOverloadedError` — capacity / high-demand signal (wins over generic 5xx unavailable)
- `ServiceUnavailableError` — temporary unavailability, including HTTP 500/502/503/504

Additional codes (auth, content policy, request limits) may be added later; unclassified failures remain `unknown`.
...
async function generateUserProfile(prompt: string) {
  ...
  try {
    ...
    if (error instanceof ProviderError) {
      if (error.code === 'rate_limit_exceeded' || error.isRetryable) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (error instanceof ProviderError && error.isRetryable) {
        ...
        console.log(`Retryable provider error (${error.code}). Retrying in ${delay}ms...`);
