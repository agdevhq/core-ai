...
- Unsupported, excessive, or non-strict-capable strict tool schemas
  (`ToolSchemaStrictnessError`)
- Invalid local request configuration

`ToolSchemaStrictnessError` includes typed `providerId`, `modelId`,
`toolNames`, `reason`, and optional `maxStrictTools` and `violations` fields.
Its `reason` is one of `'unsupported'`, `'limit-exceeded'`, or
`'invalid-schema'`. Core-ai throws it before the provider request. See the
[errors reference](/api/core/errors#toolschemastrictnesserror) for the complete
shape.
