...
| `service_unavailable` | `ServiceUnavailableError` | Temporary unavailability (e.g. 503) | yes |
| `unknown` | `ProviderError` | Unclassified provider failure | no |

**Classification precedence** (applied by each provider's `wrap*Error`):

1. Abort / cancel → `AbortedError` (not a `ProviderError`)
2. Context length
3. Overload (e.g. Anthropic HTTP 529, explicit capacity copy)
4. Rate limit (HTTP 429, provider rate-limit types)
5. Service unavailable (HTTP 503 / unavailable status)
6. Everything else → `code: 'unknown'`

Provider packages prefer structured SDK fields (status, error type/code, headers) and use message patterns only as a fallback (for example token-count extraction).
...
- `ContextLengthExceededError` — optional `maxTokens` / `actualTokens` when parseable
- `RateLimitError` — optional `retryAfterSeconds` from `Retry-After`
- `ModelOverloadedError` — strong capacity signal (not a generic 503)
- `ServiceUnavailableError` — generic temporary unavailability
