...
| `RateLimitError` | `RetryableProviderError` | HTTP 429 / provider rate limit |
...
| `ProviderError` | — | Unclassified provider failure |

Each provider `wrap*Error` helper decides which subclass to throw from SDK-specific fields (error types, status codes, message shapes). Typical order inside a wrapper: abort → context length → overload → rate limit → service unavailable / transient 5xx → generic `ProviderError`. Provider-specific quirks stay in the provider package (for example Azure capacity returning HTTP 429 as unavailable, or Vertex `RESOURCE_EXHAUSTED` as a rate limit).
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (error instanceof RetryableProviderError) {
        ...
        console.log(`Retryable provider error (${error.name}). Retrying in ${delay}ms...`);
