...
Google GenAI on Vertex AI provides Gemini chat and embedding models and Imagen image generation through regional Google Cloud endpoints.
...
```typescript
type GoogleGenAIVertexProviderOptions = {
    projectId?: string; // Required unless client is provided
    region?: string; // Required unless client is provided
    credentials?: Record<string, unknown>; // Optional: service account JSON; falls back to ADC
...
| Google GenAI on Vertex AI | ✓    | ✓          | ✓      | Gemini and Imagen on Google Cloud       |
