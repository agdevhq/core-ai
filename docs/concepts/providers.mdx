---
title: Providers
description: Learn how providers work in core-ai and how to use OpenAI, Azure OpenAI, Anthropic, Google, Vertex AI, and Mistral
---

## Overview

Providers are the bridge between core-ai and LLM services like OpenAI, Azure OpenAI, Anthropic, Google, Vertex AI, and Mistral. Each provider implements a unified interface that abstracts away provider-specific details, allowing you to switch between providers with minimal code changes.
...
<Note>
    ...
    supports chat models, while OpenAI and Google support chat, embedding,
    and image models.
</Note>
...
Google provides access to Gemini models for chat, embeddings, and image generation.
...
type GoogleProviderOptions = {
    ...
    apiVersion?: string;
    baseUrl?: string;
    client?: GoogleClient; // Optional: bring your own Google SDK client
};
```
...
Google on Vertex AI provides Gemini chat and embedding models and Imagen image generation through regional Google Cloud endpoints.
...
type GoogleVertexProviderOptions = {
    ...
    region?: string; // Required unless client is provided
    credentials?: Record<string, unknown>; // Optional: service account JSON; falls back to ADC
    client?: GoogleClient; // Optional: bring your own client
};
```
...
| Google on Vertex AI | ✓    | ✓          | ✓      | Gemini and Imagen on Google Cloud       |
