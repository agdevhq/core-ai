...
`providerOptions.omnifact` for supported Chat Completions options. Omnifact also
...
xAI provides access to Grok models. The provider uses xAI's Responses API by default and sends stateless requests with encrypted reasoning round trips.
...
const chatCompletionsModel = xai.chat.chatModel('grok-4.7');
```

Pass `providerOptions.xai` for xAI-specific options. Grok reasoning models always reason; core-ai preserves their reasoning across turns via `resultToMessage()`.
...
| xAI                    | ✓    | ✗          | ✗      | Grok reasoning models on the Responses API |
