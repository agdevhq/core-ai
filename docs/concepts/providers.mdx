---
title: Providers
description: Learn how providers work in core-ai and how to use OpenAI, Azure OpenAI, Anthropic, Vertex AI Anthropic, Google GenAI, and Mistral
---

## Overview

Providers are the bridge between core-ai and LLM services like OpenAI, Azure OpenAI, Anthropic, Vertex AI Anthropic, Google GenAI, and Mistral. Each provider implements a unified interface that abstracts away provider-specific details, allowing you to switch between providers with minimal code changes.
...
<Note>
    `createOpenAI` uses the **Responses API** by default. For the Chat
    Completions API, use `createOpenAICompat` from `@core-ai/openai/compat`.
</Note>
...
Vertex AI Anthropic provides Claude models hosted on Google Cloud, sharing its request, streaming, and reasoning behavior with the direct Anthropic provider.
...
<Note>
    ...
    A provider instance targets a single region — create separate providers
    for models hosted in different regions.
</Note>
...
| Vertex AI Anthropic | ✓    | ✗          | ✗      | Claude on Google Cloud, ADC auth        |
