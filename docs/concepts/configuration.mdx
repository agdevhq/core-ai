...
- [OpenAI](/api/providers/openai) -- Responses API (`openai.chatModel()`): `store`, `serviceTier`, `include`, `parallelToolCalls`, `user`. Chat Completions API (`openai.chat.chatModel()`): `store`, `serviceTier`, `parallelToolCalls`, `user`, `stopSequences`, `frequencyPenalty`, `presencePenalty`, `seed`
