...
The `metadata` field is application-owned. Provider adapters ignore it. The `providerMetadata` object is provider-namespaced. The top-level key is the provider identifier, such as `'anthropic'`, `'google'`, or `'openai'`.
...
- `ReasoningPart.providerMetadata` is provider-owned data used to round-trip reasoning blocks with the same provider.
- Part `metadata` is application-owned data attached to text parts, reasoning parts, tool calls, and tool results.
