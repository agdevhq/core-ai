...
Optional: `GOOGLE_VERTEX_REGION` overrides the region (default in the example: `europe-west1`); `VERTEX_ANTHROPIC_MODEL` overrides the model id (default in the example: `claude-sonnet-4-6`).
...
- `13-vertex-anthropic-provider.ts`: Using Claude on Vertex AI with the same `generate()` API
