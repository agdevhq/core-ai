...
- On known Gemini 2.5 and Gemini 3 models, an omitted `maxTokens` with
  `reasoning` set uses the verified 65,536 output limit.
...
- Requested efforts are clamped against the model's declared efforts before
  mapping. Capabilities expose `output.maxTokens` for known Gemini 2.5 and
  Gemini 3 models, plus `reasoning.thinkingBudgetRange` for Gemini 2.5.
