...
import {
    parseKimiGenerateProviderOptions,
    type KimiGenerateProviderOptions,
...
type KimiChatCompletionMessageParam =
...
export function convertMessages(messages: Message[]): KimiChatCompletionMessageParam[] {
    return messages.map(convertMessage);
}

function convertMessage(message: Message): KimiChatCompletionMessageParam {
...
function convertAssistantMessage(
...
): KimiChatCompletionMessageParam {
    ...
    for (const part of message.parts) {
        ...
        if (part.type === 'reasoning' && part.text.length > 0) {
            if (getProviderMetadata(part.providerMetadata, 'kimi')) {
...
export function validateKimiGenerateOptions(
    modelId: string,
    options: GenerateOptions
): void {
    if (!isFixedSamplingModel(modelId)) {
        return;
    }
    ...
    validateFixedSamplingValue('topP', options.topP, sampling.fixedTopP, modelId);
}
...
function createRequest(
    ...
    validateKimiGenerateOptions(modelId, options);

    const kimiOptions = parseKimiGenerateProviderOptions(options.providerOptions);
    ...
    return {
        ...
        ...(stream
            ...
            : {}),
        ...mapKimiProviderOptionsToRequestFields(kimiOptions),
    };
}

function createRequestBase(modelId: string, options: GenerateOptions) {
    validateKimiToolChoice(modelId, options.toolChoice);
    ...
    return {
        ...
        ...(options.toolChoice
            ...
            : {}),
        ...mapSamplingToRequestFields(modelId, options),
    };
}
...
function mapKimiProviderOptionsToRequestFields(
    options: KimiGenerateProviderOptions | undefined
) {
    return {
