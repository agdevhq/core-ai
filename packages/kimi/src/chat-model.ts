...
export function createKimiChatModel(
    client: KimiChatClient,
    modelId: string
): ChatModel {
    const provider = 'kimi';

    async function callKimiChatCompletionsApi<TResponse>(
        request: unknown,
        signal?: AbortSignal
    ): Promise<TResponse> {
        try {
    ...
    async function generateChat(
        ...
        const response = await callKimiChatCompletionsApi<
    ...
    async function streamChat(options: GenerateOptions): Promise<ChatStream> {
        ...
        return createChatStream(
            async () =>
                transformStream(
                    await callKimiChatCompletionsApi<
