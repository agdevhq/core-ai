...
export function createXAIChatModel(
    ...
    async function callXAIChatCompletionsApi<TResponse>(
        request: unknown,
        signal?: AbortSignal
    ): Promise<TResponse> {
        try {
    ...
    async function generateChat(
        ...
        const response = await callXAIChatCompletionsApi<
    ...
    async function streamChat(options: GenerateOptions): Promise<ChatStream> {
        ...
        return createChatStream(
            async () =>
                transformStream(
                    await callXAIChatCompletionsApi<
