...
import { getProviderContractCases } from '../contracts/provider-contract.ts';
import type { ProviderContractCase } from '../contracts/provider-contract.ts';
import type { ProviderE2EAdapter } from '../adapters/provider-adapter.ts';

const DEFAULT_CASE_TIMEOUT_MS = 120_000;

export function runProviderContractSuite(adapter: ProviderE2EAdapter): void {
    const shouldRunProviderSuite = adapter.isConfigured();
    const describeSuite = shouldRunProviderSuite ? describe : describe.skip;

    describeSuite(`${adapter.displayName} provider contract e2e`, () => {
        for (const contractCase of getProviderContractCases()) {
